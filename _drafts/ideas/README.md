...
The quantum-measurement article derives interference and path discrimination from an explicit joint state, separates quantum eraser subsets from unchanged marginals, and constructs a phase-noise counterexample with identical local statistics. It distinguishes operational predictions from interpretive questions about measurement. Two original figures and all calculations are reproduced by `assets/viz/generate_quantum_observer_figures.py --dry-run`; `tests/test_quantum_observer_models.py` checks projections, density matrices, numerical discrimination optimisation, local measurement order, phase averaging, and environment tensor products.
