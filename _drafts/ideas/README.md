...
Each new draft includes its seven-part development contract, an original figure, reproducible calculations, sources, and explicit limitations. The calculations now live in the [blog-reproducibility repository](https://github.com/DiogoRibeiro7/blog-reproducibility), one script per draft: `scripts/figures/statistics/randomisation_balance.py`, `scripts/figures/engineering/storage_ledger.py`, and `scripts/figures/health/adaptive_baseline.py`. Each takes `--dry-run` to print the synthetic calculations without writing figures. The associated tests check exact allocation probabilities, randomisation variance, energy conservation and power constraints, geometric adaptation, and missing-observation handling.
...
Each source contains a development contract. The figures and core calculations are reproduced from the [blog-reproducibility repository](https://github.com/DiogoRibeiro7/blog-reproducibility), where the shared generator has been split by subject: `scripts/figures/statistics/climate_evidence.py`, `scripts/figures/physics/seasons.py`, `scripts/figures/health/antibiotic_resistance.py`, `scripts/figures/statistics/coin_streaks.py`, and `scripts/figures/health/risk_communication.py`. Additional worked examples have executable snippets within the articles. Numerical geometry, hemisphere symmetry, conditional probabilities, selection behaviour, KL divergence, threshold information loss, overlapping runs, and heat-storage dynamics have independent checks in the matching test modules there.
...
| Archive date | Article | Source |
| --- | --- | --- |
| 2024-09-12 | Why a Million Responses Can Still Give the Wrong Answer | [Article](../../_posts/science_communication/2024-09-12-why_a_million_responses_can_still_give_the_wrong_answer.md) |
...
| 2025-01-23 | Quantum Measurement Does Not Establish That Thoughts Create Reality | [Article](../../_posts/science_communication/2025-01-23-quantum_measurement_and_the_observer_myth.md) |
...
The polling article uses a 20-million-person synthetic population to distinguish respondent precision from population accuracy. It derives selection odds and finite-population covariance, demonstrates a weighting success and failure, and constructs distinct populations with identical observed responses. Two figures and all calculations are reproduced by `scripts/figures/statistics/poll_selection.py --dry-run`; independent record-level, exhaustive-sampling, weighting, and identification-bound checks are in `tests/statistics/test_poll_selection.py`.

The p-value article derives false-positive rates, selected-study composition, likelihood comparisons, posterior model probabilities, and repeat-study rejection probabilities in an explicit Gaussian model. Two original figures and a comparison of nearly identical study estimates support its argument. Reproduce the calculations with `scripts/figures/statistics/pvalue_evidence.py --dry-run`; independent mathematical and simulation checks are in `tests/statistics/test_pvalue_evidence.py`.

The quantum-measurement article derives interference and path discrimination from an explicit joint state, separates quantum eraser subsets from unchanged marginals, and constructs a phase-noise counterexample with identical local statistics. It distinguishes operational predictions from interpretive questions about measurement. Two original figures and all calculations are reproduced by `scripts/figures/physics/quantum_observer.py --dry-run`; `tests/physics/test_quantum_observer.py` checks projections, density matrices, numerical discrimination optimisation, local measurement order, phase averaging, and environment tensor products.

The screening article follows the same 1,000 synthetic people across five scenarios to distinguish lead time, overdiagnosis, and actual postponement of death. A second model derives length-biased sampling and contrasts first-round with repeated screening. The NLST mortality result provides an empirical counterweight. Two original figures and all calculations are reproduced by `scripts/figures/health/screening_survival.py`; `tests/health/test_screening_survival.py` checks individual histories, independent interval counting, entry phases, and limiting cases.
