...
permalink: '/data-science/features/'
redirect_from:
- '/data science/features/'
seo_description: How feature discretization lets linear models capture non-linear patterns and yield deeper insight in data analysis and machine learning.
seo_title: Feature Discretization for Linear Models
seo_type: article
...
After one-hot encoding, discretization lets a linear model estimate a **step function** in the feature. That can capture threshold-like behavior, but it also discards within-bin information and creates discontinuities at bin edges. Predictive accuracy may improve or deteriorate depending on the true relationship.

### Improved Signal-to-Noise Ratio

Binning can reduce sensitivity to small measurement fluctuations when those fluctuations are genuinely irrelevant, but it can also throw away useful signal. The trade-off should be validated rather than assumed. This is particularly beneficial in datasets with a high level of measurement error or inherent variability.
...
Clinical categories may be useful for communication or policy thresholds, but discretizing a continuous biomarker for prediction can reduce statistical information and create artificial risk jumps at cut points. For instance, categorizing blood pressure readings into "normal," "elevated," and "high" can simplify the interpretation of a model predicting the risk of cardiovascular diseases, making it easier for healthcare professionals to make informed decisions.
...
Incorporating feature discretization into a machine learning pipeline involves several preprocessing steps. If bin boundaries are learned from the data—quantiles, supervised cut points, tree-based bins—they must be fitted on the training fold only. Learning boundaries on the full dataset leaks information into validation., and the resulting discrete feature is encoded, often using one-hot encoding, to prepare it for modeling.
...
Changing bin boundaries over time changes the feature definition itself. In production this can create training-serving drift and invalidate coefficient interpretation. If boundaries adapt, version them and retrain or transform the model consistently. This is particularly useful in scenarios where the data distribution evolves, such as in time-series data or in adaptive learning systems.
...
Feature discretization is one way to let a linear predictor represent stepwise nonlinear effects, allowing them to address non-linear relationships in a structured and interpretable manner. When applied thoughtfully, it can significantly improve the insights derived from a model, making it an essential technique in the data scientist's toolkit. Through careful application and consideration of both the benefits and challenges, feature discretization can bridge the gap between linear precision and non-linear complexity, providing valuable insights across various domains.

## Binning is a basis expansion

Suppose cut points are

$$
c_1<\cdots<c_K.
$$

A one-hot bin representation uses basis functions such as
...
The fitted effect is therefore piecewise constant. This makes the relationship easy to inspect, but it is less smooth and less data-efficient than alternatives such as splines.

## Supervised discretization is model selection

Choosing cut points to maximize association with the outcome uses the target. If those cut points are selected before cross-validation, performance is optimistic. Supervised binning must be nested inside the model-selection process exactly like feature selection or hyperparameter tuning.

## Compare against smooth alternatives

For nonlinear effects in linear/additive models, compare discretization with:
...
A good reason to bin is usually an interpretable domain threshold or genuine step-like mechanism—not merely the desire to make a continuous variable categorical.
