...
The fitted effect is therefore piecewise constant.

This makes the relationship easy to inspect, but it is less smooth and less data-efficient than alternatives such as splines.

## Supervised discretization is model selection

Choosing cut points to maximize association with the outcome uses the target.

If those cut points are selected before cross-validation, performance is optimistic.

Supervised binning must be nested inside the model-selection process exactly like feature selection or hyperparameter tuning.
