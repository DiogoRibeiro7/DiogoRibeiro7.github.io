...
A laptop and DuckDB are enough to apply all of this to tens of gigabytes, and the same rules hold at petabytes, where the engine has a different name and the bill for ignoring them has more digits. The benchmark is [`data_lake_benchmarks.py`](https://github.com/DiogoRibeiro7/DiogoRibeiro7.github.io/blob/master/assets/viz/data_lake_benchmarks.py) in this site's repository, with its results file beside it. It writes about a gigabyte of temporary files, takes a few minutes, and declines to time anything while the machine is busy.
