...
The [figure generator](https://github.com/DiogoRibeiro7/DiogoRibeiro7.github.io/blob/master/assets/viz/generate_2026_evidence_articles.py) reproduces the release timeline and decision table from the [sourced CSV](/assets/data/gdp_q1_2025_release_vintages.csv). Three rows are enough to expose the problem. A larger backtest needs to preserve the same information history for every input on which its conclusions depend.
