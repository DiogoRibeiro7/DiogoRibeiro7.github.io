...
- python
- plaintext
permalink: '/machine-learning/understanding_fowlkes_mallows_index/'
redirect_from:
- '/machine learning/understanding_fowlkes_mallows_index/'
...
The Fowlkes-Mallows Index was developed as an external measure of agreement between two partitions of the same observations. Its key idea is pairwise rather than cluster-wise: for every pair of observations, each partition either places the pair together or separates it. The FMI then summarizes how consistently those co-membership decisions agree. This makes the measure useful when comparing two clustering algorithms or a clustering result against an external reference partition, while avoiding any dependence on the arbitrary numeric labels assigned to the clusters.

### Applications in Clustering and Classification

The same calculation can be applied to predicted class labels and known labels, but that does not make FMI a substitute for ordinary classification metrics. Supervised class labels have semantic meaning, whereas clustering labels are permutation-invariant identifiers. In classification work, case-wise errors, calibration, class imbalance, and decision costs are usually more informative than pairwise partition agreement.
...
The main value of FMI is its compact pairwise interpretation, but the same construction also creates its limitations:
...
Applications of FMI are most defensible when an external partition exists and pairwise co-membership is the quantity of interest. In practice, the score should be interpreted together with domain-specific diagnostics rather than presented as a generic certificate of clustering quality.
...
def generate_pairs(labels):
    ...
    each pair belongs to the same cluster or not. Args:
        labels (list or array): List or array of cluster labels for elements. Returns:
...
Args:
        ...
        pred_labels (list or array): Predicted cluster labels for each element. Returns:
