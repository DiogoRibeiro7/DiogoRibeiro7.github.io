...
$$
When stepping backward from maturity it is clearer to index time-to-maturity $\tau=T-t$. The explicit update has the form
...
with a center coefficient containing the identity term. Omitting that $1$ changes the scheme entirely.
$$
...
For a European call under the standard Black-Scholes assumptions, an analytic solution exists. That makes this problem ideal for numerical verification.

A correct FDM implementation should demonstrate that
