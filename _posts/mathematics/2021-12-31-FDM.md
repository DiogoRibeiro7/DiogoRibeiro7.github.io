---
permalink: '/mathematics/FDM/'
author_profile: false
categories:
- Mathematics
classes: wide
date: '2021-12-31'
excerpt: Explore how Finite Difference Methods and the Black-Scholes-Merton differential
  equation are used to solve option pricing problems numerically, with a focus on
...
- Option pricing
- Stability analysis
seo_description: How Finite Difference Methods solve the Black-Scholes-Merton equation for option pricing, covering explicit and implicit schemes and stability.
seo_title: Finite Difference Methods in Option Pricing
seo_type: article
summary: This article explains how Finite Difference Methods (FDM) are applied to
...
When stepping backward from maturity it is clearer to index time-to-maturity $\tau=T-t$. The explicit update has the form $ V_i^{m+1} = a_iV_{i-1}^{m} + b_iV_i^{m} + c_iV_{i+1}^{m}, $ with a center coefficient containing the identity term. Omitting that $1$ changes the scheme entirely. While straightforward, the explicit scheme is **conditionally stable**, meaning that the time step $$\Delta t$$ must be sufficiently small to ensure accuracy and avoid divergence.
...
The backward-Euler implicit scheme is unconditionally stable in the usual linear stability sense, but large time steps can still produce substantial discretization error. Stability is not the same thing as accuracy.
...
# scipy.linalg.solve_banded expects compact banded storage:
...
# Move known boundary contributions to the right-hand side.
    rhs[0] += alpha[1] * V[m-1, 0]
    rhs[-1] += gamma[N-1] * V[m-1, N]

V[m-1, 1:N] = la.solve_banded(
        (1, 1),
        ab,
        rhs,
    )
...
A simple global CFL-like bound can be used as a conservative check, but stability for the variable-coefficient Black-Scholes grid is better assessed from the actual update coefficients. Requiring nonnegative explicit weights gives a practical sufficient condition, and convergence should be verified by grid refinement rather than by one inequality alone. Here's a snippet for checking the stability condition:
...
Both implementations are useful for pricing European options under the Black-Scholes-Merton framework and can be extended to more complex scenarios, such as American options, by incorporating early exercise conditions. By running the code provided, you can numerically solve the Black-Scholes equation and visualize the option prices for different asset prices using both finite difference methods.

### Crank-Nicolson

A common compromise is Crank-Nicolson, which averages the spatial operator between adjacent time levels:

$$
\frac{V^{m+1}-V^m}{\Delta t}
=
\frac12
\left[
L V^{m+1}
...
It is second-order accurate in time for smooth solutions and is widely used for European options. Near the payoff kink, Rannacher smoothing or a few backward-Euler startup steps can reduce oscillations.

### Validate against the analytic solution

For a European call under the standard Black-Scholes assumptions, an analytic solution exists. That makes this problem ideal for numerical verification. A correct FDM implementation should demonstrate that

$$
\|V_{\Delta S,\Delta t}-V_{BS}\|
\rightarrow0
$$

as the grid is refined at the expected rate. Without this convergence check, a visually plausible option curve is not enough evidence that the discretization is correct.
