...
and flow conservation at intermediate nodes, maximize total source-to-sink flow. The max-flow/min-cut theorem gives an exact structural interpretation: the optimal flow equals the capacity of a minimum cut.
...
subject to node balances and arc capacities. This model is directly relevant to transportation, production routing, and multi-stage distribution.
...
A tree is fragile because removing one edge disconnects the network. If resilience matters, model failure scenarios, backup routes, capacity reserve, or $k$-connectivity explicitly. The cheapest nominal network and the most robust network are usually different solutions.
