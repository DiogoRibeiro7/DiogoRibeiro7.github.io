...
The average pseudovalue is related to the bias-corrected jackknife estimate.

This representation is useful for understanding why the method works best for smooth statistics whose leave-one-out perturbations are small.

## When the jackknife can fail

The jackknife can perform poorly for non-smooth estimators such as sample extrema or statistics with abrupt model-selection changes.

For the sample median, behavior depends strongly on sample size and distributional smoothness.

The bootstrap is often more flexible, but it also has failure modes.

Choose the resampling method from the statistic and sampling design rather than from a generic hierarchy of “robustness.”
