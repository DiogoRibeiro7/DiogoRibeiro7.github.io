"""Fetch freely licensed images from Wikimedia Commons as header images.
...
QUERIES = {
    ...
    "control-room": "control room computer monitors operations",
    "data-center": "data center server racks",
    "factory":     "manufacturing robot line",
    "microscope":  "laboratory microscope science research",
    "motherboard": "motherboard close-up",
    "network-cables": "network rack cables patch panel",
    "radio-telescope": "radio telescope observatory dish",
    "satellite-dish": "satellite dish antenna sky",
    "solar-panels": "solar panels renewable energy",
    "wind-turbines": "wind turbines renewable energy landscape",
    "wafer":       "silicon wafer",
...
def to_rgb(img):
    if img.mode in ("RGBA", "LA") or "transparency" in img.info:
        ...
        return Image.alpha_composite(canvas, img.convert("RGBA")).convert("RGB")
...
def fetch(slug, query):
    for c in candidates(query):
        ...
        with urllib.request.urlopen(req, timeout=120) as r:
            img = to_rgb(Image.open(io.BytesIO(r.read())))
